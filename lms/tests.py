...
class LessonTestCase(APITestCase):
    ...
    def test_retrieve_lesson_by_owner(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse('lms:lesson-retrieve', kwargs={'pk': self.lesson.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_lesson_by_owner(self):
        ...
        response = self.client.patch(reverse('lms:lesson-update', kwargs={'pk': self.lesson.pk}), data=data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
