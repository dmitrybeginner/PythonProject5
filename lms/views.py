...
@method_decorator(name='list', decorator=swagger_auto_schema(
    operation_description="Просмотр списка курсов. Модераторы видят все курсы, остальные пользователи - только свои."
))
@method_decorator(name='retrieve', decorator=swagger_auto_schema(
    operation_description="Просмотр одного курса. Доступно модераторам и владельцу курса."
))
@method_decorator(name='create', decorator=swagger_auto_schema(
    operation_description="Создание нового курса. Доступно всем аутентифицированным пользователям, кроме модераторов."
))
...
@method_decorator(name='list', decorator=swagger_auto_schema(
    operation_description="Просмотр списка уроков. Модераторы видят все уроки, остальные пользователи - только свои."
))
@method_decorator(name='retrieve', decorator=swagger_auto_schema(
    operation_description="Просмотр одного урока. Доступно модераторам и владельцу урока."
))
@method_decorator(name='create', decorator=swagger_auto_schema(
    operation_description="Создание нового урока. Доступно всем аутентифицированным пользователям, кроме модераторов."
))
@method_decorator(name='update', decorator=swagger_auto_schema(
