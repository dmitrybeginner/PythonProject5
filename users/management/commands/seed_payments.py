...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        lesson, created = Lesson.objects.get_or_create(
            name="Introduction to Models", course=course
        )
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Successfully created lesson: {lesson.name}")
            )
        else:
            self.stdout.write(
        ...
        for payment_data in payments_to_create:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully created payment: {payment}")
            )
